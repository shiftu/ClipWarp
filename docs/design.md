...
- **M4 静态加密**：clip `content`/`title` 以 AES-256-GCM 加密落盘（每条随机 IV + 认证标签）；主密钥 `master.key` 存于 `~/.config/clipwarp/`（chmod 600，与 `data/` 分目录）；服务启动时执行一次性迁移，将存量明文自动加密，并前置密钥自检（错密钥 fail-fast，不损坏数据）。
